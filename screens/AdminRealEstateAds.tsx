...
import { 
  ChevronLeft, Search, Home, CheckCircle, XCircle, AlertTriangle, 
  MapPin, User as UserIcon, Filter, Clock, ChevronRight,
  Info, FileText, Bed, Bath, Maximize, Car
} from 'lucide-react';
import { AdItem, AdStatus } from '../types';
import { MOCK_ADMIN_REAL_ESTATE } from '../constants';
import { PriceTag } from '../components/Shared';

interface AdminRealEstateAdsProps {
  onBack: () => void;
}
...
export const AdminRealEstateAds: React.FC<AdminRealEstateAdsProps> = ({ onBack }) => {
  const [ads, setAds] = useState<AdItem[]>(MOCK_ADMIN_REAL_ESTATE);
  ...
  const filteredAds = useMemo(() => {
    return ads.filter(ad => {
      ...
      if (searchTerm) {
        ...
        const matchesType = ad.realEstateType?.toLowerCase().includes(lowerSearch);
  ...
  if (selectedAd) {
    ...
    // Cálculo simples de preço por m² (apenas visualização)
    const pricePerSqm = selectedAd.area && selectedAd.price ? selectedAd.price / selectedAd.area : 0;
    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-24 animate-in slide-in-from-right duration-300 relative">
        ...
        <div className="bg-white px-4 py-4 sticky top-0 z-40 flex items-center justify-between border-b border-gray-200 shadow-sm">
           ...
           <h1 className="text-lg font-bold text-gray-900">Moderação de Imóvel</h1>
        ...
        <div className="pb-24">
           ...
           <div className="relative h-64 bg-gray-900">
              <img src={images[currentImageIndex]} className="w-full h-full object-cover" alt="Imóvel" />
           ...
           <div className="p-5 space-y-6">
              ...
              {/* Title & Price */}
              <div>
                 <span className="text-xs font-bold text-primary bg-blue-50 px-2 py-1 rounded mb-2 inline-block">
                    {selectedAd.realEstateType || 'Imóvel'}
                 </span>
                 ...
                 {/* Price Analysis Box */}
                 <div className="mt-3 p-3 rounded-xl border bg-gray-50 border-gray-100 flex items-start gap-3">
                    <Info className="w-5 h-5 flex-shrink-0 mt-0.5 text-gray-400" />
                    <div>
                       <p className="text-xs font-bold uppercase opacity-80 text-gray-500">Métrica de Valor</p>
                       <p className="font-bold text-sm text-gray-800">
                          {pricePerSqm > 0 ? `R$ ${pricePerSqm.toLocaleString('pt-BR', {maximumFractionDigits: 0})}/m²` : 'Área não informada'}
                       </p>
                    </div>
                 </div>
              </div>
              ...
              <div>
                 <h3 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
                    <Home className="w-4 h-4 text-gray-500" /> Detalhes do Imóvel
                 </h3>
                 <div className="grid grid-cols-2 gap-3">
                    <SpecItem icon={<Maximize className="w-3.5 h-3.5" />} label="Área Total" value={selectedAd.area ? `${selectedAd.area} m²` : '-'} />
                    <SpecItem icon={<Bed className="w-3.5 h-3.5" />} label="Quartos" value={selectedAd.bedrooms || '-'} />
                    <SpecItem icon={<Bath className="w-3.5 h-3.5" />} label="Banheiros" value={selectedAd.bathrooms || '-'} />
                    <SpecItem icon={<Car className="w-3.5 h-3.5" />} label="Vagas" value={selectedAd.parking || '-'} />
                    <SpecItem icon={<MapPin className="w-3.5 h-3.5" />} label="Local" value={selectedAd.location} />
                    <SpecItem label="Tipo" value={selectedAd.realEstateType || 'Outro'} />
                 </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 animate-in slide-in-from-right duration-300">
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200 px-4 py-4 flex items-center gap-3 shadow-sm">
        ...
        <h1 className="text-xl font-bold text-gray-900">Gerenciar Imóveis</h1>
      </div>
               ...
               >
                  ...
                  <div className="flex items-center justify-between px-4 py-3 bg-gray-50/50 text-xs text-gray-600">
                     <div className="flex items-center gap-1">
                        <Maximize className="w-3.5 h-3.5" /> {ad.area}m²
                     </div>
